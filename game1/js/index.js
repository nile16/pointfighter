...
(function getUserData(){
	...
	xhr.onreadystatechange = function(){
		if(this.readyState == 4 && this.status == 200) {
				...
				if (user.err)
					board.innerHTML = "<div style='text-align:center;font-size:30px;color:blue;'>Error: "+user.err+"</div>";
				else
					newgame();
		}
	}
...
function checkForMoveVictory(){
  if(!checkForVictory(currentRow,currentCol)){
    placeDisc(3-currentPlayer);
	  ...
	  if (currentPlayer==2){
		board.innerHTML = "<div style='text-align:center;font-size:30px;color:blue;'>You lost 10 health points</div>";
		user.hea-=10;
		if (user.hea<0) user.hea=0; 
		updateUserData({hea:user.hea});
	  }
	  else{
		board.innerHTML = "<div style='text-align:center;font-size:30px;color:blue;'>You won 1 level</div>";
		if (user.lev<100) user.lev+=1;
		updateUserData({lev:user.lev});
	  }
  }
}
